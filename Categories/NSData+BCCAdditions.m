...
@implementation NSData (BCCAdditions)
...
- (NSData *)BCC_SHA1Value
{
    ...
    void* buffer = malloc(CC_SHA1_DIGEST_LENGTH);
    CC_SHA1(dataBuffer, (CC_LONG)self.length, buffer);
    return [NSData dataWithBytesNoCopy:buffer length:CC_SHA1_DIGEST_LENGTH];
}
...
@implementation NSMutableData (BCCAdditions)
...
- (void)BCC_appendUTF8StringWithFormat:(NSString *)inString, ...
{
    ...
    [self BCC_appendUTF8StringWithFormat:inString arguments:args];
...
- (void)BCC_appendUTF8StringWithFormat:(NSString *)inString arguments:(va_list)inArguments
{
    ...
    [self BCC_appendUTF8String:formattedString];
}

- (void)BCC_appendUTF8String:(NSString *)inString;
{
    [self BCC_appendString:inString withEncoding:NSUTF8StringEncoding];
}
...
+ (NSData *)BCC_dataFromBase64String:(NSString *)aString
{
	...
	size_t outputLength;
	void *outputBuffer = BCCNewBase64Decode([data bytes], [data length], &outputLength);
...
- (NSString *)BCC_base64EncodedString
{
	size_t outputLength;
	char *outputBuffer =
    BCCNewBase64Encode([self bytes], [self length], true, &outputLength);
